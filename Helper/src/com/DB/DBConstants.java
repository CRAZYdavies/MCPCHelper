...
public class DBConstants {
	...
	public static final String TVEPISODESIPT_TABLE_CREATE ="create table if not exists 'tvepisodesipt' ('tid' integer not null,'showname' varchar(50),'torrentlink' varchar(500) not null,'dateuploaded' integer,'downloaded' char(1) default 'n','episode' integer,'season' integer,'tvdbid' varchar(64),'aireddate' integer,'episode_name' varchar(50),'file_size' varchar(12),'resolution' varchar(8),primary key(tid))";
	...
	public static final String INSERT_TVEPISODESIPT_1 = "insert into tvepisodesipt (tid,showname,torrentlink,dateuploaded,downloaded,episode,season,tvdbid,aireddate,episode_name,file_size,resolution) values (";
	...
	public static final String GET_ALL_SHOWS = "select showname,md5_checksum,magnetlink,dateuploaded from tvepisodes order by dateuploaded desc LIMIT 100";
	public static final String FIND_SHOW = "select * from tvepisodes where showname like '%~1%'";
	public static final String GET_SEASON = "select * from tvepisodes where season = (select season from tvepisodes where md5_checksum = '~1') and showname = (select showname from tvepisodes where md5_checksum = '~1')";
}
